            ...
            *[
                nn.Sequential(
                    ...
                    wrapper(
                        i,
                        nn.Sequential(
                            nn.Linear(mlp_in_dim,
                                      mlp_in_dim * expansion,
                                      bias=False), active_func,
                            nn.Dropout(p=dropout, inplace=True),
                            nn.Linear(mlp_in_dim * expansion,
                                      mlp_in_dim,
                                      bias=False)))) for i in range(layer_num)
            ],
            Affine(mlp_in_dim))
...
class SimpleMLPReg(nn.Module):
    ...
    def __init__(self,
        ...
        self.net = nn.Sequential(Rearrange('b l f -> b (l f)'), *[
            nn.Sequential(nn.Linear(self.input_len, self.input_len),
                          active_fun, self.dropout) for i in range(layer_num)
        ]
...
class TwoLayerModel(nn.Module):
    ...
    def __init__(self, model_para=None):
        ...
        if model_para["reg"]["name"] == "MLP":
            self.reg = SimpleMLPReg(patch_num=patch_num,
